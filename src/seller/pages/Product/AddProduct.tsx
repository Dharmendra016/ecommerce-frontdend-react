...
const AddProduct = () => {
  ...
  const hadleImageChange = async (event: any) => {
    ...
    // Correct the way files are accessed
    const files = event.target.files; // It is files, not file
    if (files) {
      Array.from(files).map(async (file: any) => {
        setUploadingImage(true); // Set image uploading state to true
        const image = await uploadToCloudinary(file); // Upload the image
        formik.setFieldValue("images", [...formik.values.images, image]); // Update form values with new image
        setUploadingImage(false); // Reset image uploading state to false
      });
    }
  }
        ...
        <Grid2 container spacing={2}>
          ...
          <Grid2 className="flex flex-wrap gap-5" size={{ xs: 12 }}>
            <input
              ...
              id="fileInput"
              style={{ display: "none" }}
              onChange={hadleImageChange}  // Fixed typo
            />
